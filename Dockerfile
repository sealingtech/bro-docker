...
RUN yum -y update && \
        yum -y install cmake make gcc gcc-c++ flex bison libpcap-devel openssl-devel python-devel swig zlib-devel GeoIP-devel libpcap gperftools vim wget kernel-headers kernel-devel && \
        ...
        make && \
        make install &&\
        yum -y erase libpcap-devel openssl-devel python-devel zlib-devel GeoIP-devel wget && \
